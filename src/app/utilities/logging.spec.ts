...
import { configureMockEnvVars, getMockInstance } from '../_test_utils';
...
describe('makeLogger', () => {
  ...
  test('APP_VERSION env var should be required', () => {
    mockEnvVars({ ...REQUIRED_ENV_VARS, APP_VERSION: undefined });
  ...
  test('APP_VERSION should be passed to cloud logging config', () => {
    makeLogger();

    expect(getPinoOptions).toBeCalledWith(
      undefined,
      expect.objectContaining({
        version: REQUIRED_ENV_VARS.APP_VERSION,
      }),
    );
  });
